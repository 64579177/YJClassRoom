...
//        }
        ...
        let with:CGFloat
        let heigth:CGFloat
        if (self.resultImgSize?.width)! > kscWidth{
            heigth = kscWidth/kscHeight * (self.resultImgSize?.height)!
            y = (kscHeight - heigth)/2
            x = 0
            with = kscWidth
        }else{
            y = (kscHeight - (self.resultImgSize?.height)!)/2
            x = (kscWidth - (self.resultImgSize?.width)!)/2
            with = (self.resultImgSize?.width)!
            heigth = (self.resultImgSize?.height)!
        }
        
        clipperView = UIImageView(frame: CGRect(x: x, y: y, width:with , height:heigth ))
